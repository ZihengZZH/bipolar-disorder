...
def save_cv_results(cv_res, model_name, feature_name, modality):
    """save Cross Validation results to external files
    """
    # para cv_res: cross validation results
    ...
    if modality == 'baseline':
        filename = os.path.join(data_config['result_baseline'], '%s_%s_result.txt' % (model_name, feature_name))

    elif modality == 'single':
        filename = os.path.join(data_config['result_single'], '%s_%s_result.txt' % (model_name, feature_name))
    
    elif modality == 'multiple':
        filename = os.path.join(data_config['result_multi'], '%s_%s_result.txt' % (model_name, feature_name)) 

        with smart_open(filename, 'a+', encoding='utf-8') as f:
            f.write("CV results on session-level\n")
            for res in cv_res:
                f.write("%.3f; " % res)
            f.write("\n")
        f.close()
